...
def test_post_positive():
    ...
    response = requests.post(url,
                             json=request)  # По переменной url , делает json запрос и сохраняет его в response (id name и тп)
    print(
        "==================================================RESPONSE POST=====================================================================================")
    ...
    while responseGet.json()['name'] != "KOTOSBER":
        print(
            'Данные не обновились, пробуем еще раз')  # Это необходимо тк база не зеркалируется, что бы избежать неверного ответа.
    ...
    responsePut = requests.put(urlPut,
                               json=request)  # По переменной url , делает json запрос и сохраняет его в response (id name и тп)
    print(
        "===================================================RESPONSE PUT================================================================================")
    ...
    while responseGet.json()[
        'name'] != "SBERDOG":  # Запускаем Цикл, который будет выполняться пока name не станет Sberdog.
        print(
            'Данные не обновились, пробуем еще раз')  # Это необходимо тк база не зеркалируется, что бы избежать неверного ответа.
    ...
    print(
        "===================================================REQUEST DELETE=================================================================================")
    urlDel = "https://petstore.swagger.io/v2/pet/" + str(
        response.json()['id'])  # Тк id в данном случает int, то привели к типу str - строковый
    ...
    assert responseDel.json()['message'] == str(
        responsePut.json()['id'])  # Проверяем что message из Delete это ID из Put.
    ...
    while responseGetNeg.json()[
        'status'] != "TROLL":  # Запускаем Цикл, который будет выполняться пока name не станет Sberdog.
        print(
            'Данные не обновились, пробуем еще раз')  # Это необходимо тк база не зеркалируется, что бы избежать неверного ответа.
